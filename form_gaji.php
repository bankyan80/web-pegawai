...
if (isset($_SESSION['MEMBER']) && $_SESSION['MEMBER']['role'] != 'staff') {
  ...
  //mode edit: pegawai pada gaji tidak boleh diganti
  $dis = !empty($idedit) ? 'disabled' : '';
?>
  <form method="POST" action="controller_gaji.php">
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        <select id="nama" name="nama" class="custom-select" <?= $dis ?>>
          ...
          ?>
              <option value="<?= e($peg['id']) ?>" selected><?= e($peg['nama']) ?></option>
            <?php }
          }
          foreach ($pegawai2 as $peg2) {
            ?>
            <option value="<?= e($peg2['idp']) ?>"><?= e($peg2['nama']) ?></option>
          <?php }
          ?>
        </select>
      </div>
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        <input id="gapok" value="<?= e($rs['gapok']) ?>" name="gapok" type="text" class="form-control">
      </div>
    </div>
    <div class="form-group row">
      <label for="tunjab" class="col-4 col-form-label">Tunjangan Jabatan</label>
      <div class="col-8">
        <input id="tunjab" name="tunjab" value="<?= e($rs['tunjab']) ?>" type="text" class="form-control">
      </div>
    </div>
    <div class="form-group row">
      <label for="bpjs" class="col-4 col-form-label">BPJS</label>
      <div class="col-8">
        <input id="bpjs" name="bpjs" value="<?= e($rs['bpjs']) ?>" type="text" class="form-control">
      </div>
    </div>
    <div class="form-group row">
      <label for="lain2" class="col-4 col-form-label">Lain-Lain</label>
      <div class="col-8">
        <input id="lain2" name="lain2" value="<?= e($rs['lain2']) ?>" type="text" class="form-control">
      </div>
    </div>
    <div class="form-group row">
